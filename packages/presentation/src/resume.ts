import type { ResumeWork, ResumeCandidate, ResumeCorrection, Continuation, ContinuationPayload, Receipt } from '@statecarry/contracts';
export type { ResumeWork, ResumeCandidate, ResumeCorrection, Continuation } from '@statecarry/contracts';
export interface ResumeGateway {
 ...
 refresh(workId: string): Promise<void>;
  ...
  prepareContinuation?(workId: string, revision: number, input: { targetMode: 'new-session' | 'existing-session'; threadId?: string | null; payload: ContinuationPayload }): Promise<Continuation>;
 ...
 */
export type ResumeProgressViewModel = {
  completed: string;
  remaining: string;
};
...
export function presentResumeProgress(candidate: ResumeCandidate): ResumeProgressViewModel {
  ...
  if (candidate.status === 'active') {
    ...
    remaining = hasVerified ? 'No further action is recorded in this brief.' : 'Independent verification is still needed before treating this as finished.';
  }
 ...
 */
export function continuationPayload(candidate: ResumeCandidateViewModel, work: ResumeWork): ContinuationPayload | null {
 ...
 // stale action text.
 if (candidate.status !== 'active' || candidate.actionAvailable === false || !candidate.nextAction || !candidate.doneWhen) return null;
  const evidence = [
    ...candidate.evidence,
...
export function resumeStatusLabel(status: ResumeCandidate['status']) { return statusLabels[status]; }
export function resumeActionSourceLabel(source: ResumeCandidate['actionSource']) { return source ? actionSourceLabels[source] : null; }
export function resumeRoleLabel(role: string | null | undefined) { return role ? roleLabels[role] ?? 'Connected conversation' : 'Connected conversation'; }
export function resumeActorLabel(actor: string | null | undefined) { return actor ? actorLabels[actor] ?? 'Conversation record' : 'Conversation record'; }
export function resumeUtteranceTypeLabel(type: string | null | undefined) { return type ? utteranceTypeLabels[type] ?? 'Conversation record' : 'Conversation record'; }
/** Alias for callers that use the contracts' existing "nature" name. */
export function resumeNatureLabel(nature: string | null | undefined) { return resumeUtteranceTypeLabel(nature); }
...
function targetFor(candidate: ResumeCandidate, work: ResumeWork): ResumeTargetViewModel {
 ...
 const currentEnough = !work.stale && !work.updatesAvailable && !work.workspaceChanged && !work.busy && !work.error;
 const sessionReady = candidate.status === 'active' && !!candidate.nextAction && !!candidate.doneWhen
  && currentEnough && resumeWorkStatus(work).canAct
  && work.session?.create === 'supported' && work.session.send === 'supported';
 ...
 const navigationReady = work.navigation
  ? work.navigation.precision === 'thread' && (!work.navigation.state || work.navigation.state === 'verified-route')
  : true;
  const coordination = work.coordination ?? null;
  const coordinationReady = navigationReady && coordination?.state === 'recommended' && !!coordination.threadId;
  return {
    existing: {
      mode: 'existing-conversation', available: navigationReady, ...(navigationReady ? { url } : {}),
    ...
    newSession: {
      mode: 'new-session', available: sessionReady,
      ...
      detail: !currentEnough
        ...
        : sessionReady
          ...
          : (work.session?.detail ?? 'Creating a new session is not connected here. Use the recorded conversation to keep the existing work together.'),
    },
    coordination: {
      mode: 'coordination-conversation',
      available: coordinationReady,
      ...(coordinationReady ? { url: `codex://threads/${encodeURIComponent(coordination!.threadId!)}` } : {}),
      label: coordination?.state === 'recommended' ? 'Open coordination conversation' : 'Check for a coordination conversation',
      detail: coordination?.detail ?? 'The role of each connected conversation has not been confirmed.',
    },
  };
}
...
function candidateView(candidate: ResumeCandidate, work: ResumeWork): ResumeCandidateViewModel {
 ...
 return {
  ...
  actionAvailable: candidate.status === 'active' && !!candidate.nextAction && !!candidate.doneWhen && !!candidate.actionSource
  && status.canAct,
...
const workStateDescriptions: Record<ResumeWorkState, string> = {
 ready: 'A current resume brief is ready. Review the next action and its completion condition before starting.',
 checking: 'StateCarry is checking the connected records for a safe place to resume. Actions that depend on this check are paused.',
 limited: 'A previous brief is available, but some current project or record checks are missing. Review the limits before acting.',
 empty: 'No resume candidate was found in the connected records yet. Check the connection or add the missing records, then try again.',
 unavailable: 'The connected records are unavailable, so StateCarry cannot establish where to resume. Check the connection and try again.',
 failed: 'The latest records could not be checked. Your previous brief is kept; retry the check or open the recorded conversation to inspect it.',
};
...
function workLimitations(work: ResumeWork): string[] {
 const values = [
  ...
  ...(work.workspace && work.workspace.status !== 'checked' ? ['The current project state could not be confirmed.'] : []),
  ...(work.workspaceChanged ? ['The project changed since this brief was captured.'] : []),
  ...(work.updatesAvailable ? ['New connected records arrived after this brief was captured.'] : []),
  ...(work.stale ? ['This brief needs a fresh check before its next action can be trusted.'] : []),
  ...(work.generatedAt === null && work.candidates.length ? ['The time this brief was checked is unavailable.'] : []),
 ];
...
export function resumeWorkState(work: ResumeWork): ResumeWorkState { return resumeWorkStatus(work).state; }
export function resumeStateLabel(state: ResumeWorkState): string { return workStateLabels[state]; }
export function resumeStateDescription(state: ResumeWorkState): string { return workStateDescriptions[state]; }
...
export function presentCoordination(work: ResumeWork): CoordinationViewModel {
 ...
 if (!coordination || coordination.state === 'none') return {
  state: 'none', label: 'No coordination conversation selected',
  description: 'The connected conversations do not have a confirmed overall-progress role.',
  available: false, threadId: null, title: null, choices: work.coordinationChoices ?? [], evidence: [],
 };
 if (coordination.state === 'recommended') return {
  state: 'recommended', label: 'Coordination conversation',
  description: coordination.detail, available: !!coordination.threadId,
  threadId: coordination.threadId, title: coordination.title, choices: work.coordinationChoices ?? [],
  evidence: coordination.evidence,
 };
 return {
  state: 'unconfirmed', label: 'Coordination role unconfirmed',
  description: coordination.detail, available: false,
  threadId: null, title: coordination.title, choices: work.coordinationChoices ?? [],
  evidence: coordination.evidence,
 };
}
...
function continuationTextFromPayload(payload: ContinuationPayload): string {
 ...
 if (payload.goalConfirmed === false) lines.splice(2, 0, 'Goal status: Inferred from connected records; confirm it before acting.');
 ...
 lines.push('Confirm the current state before changing files, then report the result against the done-when condition.');
 return lines.join('\n');
}
 ...
 * continuation path.
 */
export function manualContinuation(candidate: ResumeCandidateViewModel, work: ResumeWork): ManualContinuation | null {
...
export function manualContinuationText(candidate: ResumeCandidateViewModel, work: ResumeWork): string | null {
 ...
 */
export function manualReviewContinuationText(candidate: ResumeCandidateViewModel, work: ResumeWork): string {
...
export function continuationText(candidate: ResumeCandidateViewModel, work: ResumeWork): string | null;
export function continuationText(first: ContinuationPayload | ResumeCandidateViewModel, work?: ResumeWork): string | null {
