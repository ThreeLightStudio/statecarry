...
export function presentFlowGraph(flow: FlowView): FlowGraph {
  ...
  for (const [i, item] of flow.items.entries()) {
    ...
    for (const ref of item.sources) {
      if (!input.has(ref.revisionId) || !ref.available || !ref.quote || !ref.quoteStarts?.length)
        continue;
      const members = shared.get(ref.revisionId) ?? new Set<number>();
      members.add(i);
      shared.set(ref.revisionId, members);
    }
  }
  const edges: string[] = [];
  for (const [revisionId, members] of [...shared].sort(([a], [b]) =>
    a < b ? -1 : a > b ? 1 : 0,
  )) {
    ...
    nodes.push({
      id,
      label: `Shared source ${number}`,
      revisionId,
      itemIds: [...members].map((i) => flow.items[i].id),
    });
    for (const member of members) edges.push(`n${member} --- ${id}`);
  }
  const result: FlowGraph = {
    nodes,
    connected: edges.length > 0,
    limitation: edges.length
      ? null
      : 'No shared sources can be linked. Items and coverage are available as text.',
    source: [
      'flowchart TB',
      ...nodes.map((n) => `${n.id}["${graphLabel(n.label)}"]`),
      ...edges,
    ].join('\n'),
  };
