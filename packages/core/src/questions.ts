...
export class ContextQuestions {
  ...
  private fail(message: string): never {
    throw new DomainError('SOURCE_UNAVAILABLE', message, 409);
  }
  private execution(id: string) {
    return this.core.repo.get('questionExecution', id);
  }
  private save(e: QuestionExecution) {
    this.core.repo.put('questionExecution', { ...e, updatedAt: this.now() });
  }
  private replay(id: string, hash: string) {
    ...
    if (prior && prior.bodyHash !== hash)
      throw new DomainError(
        ...
        'This request ID already has a different question.',
        409,
      );
    return prior;
  }
  ...
  private anchor(s: QuestionSession) {
    ...
    const claim = this.core.repo
      .get('summary', s.summaryId)
      ?.claims.find((c) => c.id === s.claimId);
  ...
  private scope(s: QuestionSession): SourceRevision[] {
    ...
    if (
      ...
      sources.some(
        (source) =>
          !source || !linked.has(source.threadId) || !this.core.accessibleSource(w.id, source.id),
      )
    ) {
      s.invalidated = true;
      s.anchor = 'Source scope changed';
      for (const turn of s.turns) {
        turn.answer = null;
        turn.assessment = null;
        if (active(turn.status)) turn.status = 'invalidated';
  ...
  create(workId: string, value: unknown) {
    ...
    if (
      summary.sourceRevisionIds.some((id) => {
        ...
        return !source || !allowedKeys.has(source.key);
      })
    )
      this.fail(
        'Summary sources are outside the current collection scope. Check the connection scope and use a new summary.',
      );
  ...
  submit(workId: string, id: string, value: unknown) {
    ...
    this.canRun(s);
    if (
      s.turns.length >= QUESTION_LIMITS.turns ||
      JSON.stringify(questionHistory(s)).length + input.text.length > QUESTION_LIMITS.history
    )
      this.fail('This question session reached its exchange limit. Start a new session.');
  ...
  retry(workId: string, id: string, turnId: string, value: unknown) {
    ...
    if (!t || t.id !== turnId || !t.retryable || t.attempts >= 2 || t.status !== 'failed')
  ...
  private canRun(s: QuestionSession) {
    if (this.closing || !this.core.summary.answerQuestion || !this.core.summary.checkQuestion)
      throw new DomainError('CAPABILITY_UNSUPPORTED', 'The question model is unavailable.');
    if (s.turns.some((t) => active(t.status)))
      this.fail('Confirm that the previous question has finished first.');
    if (
      this.hasUnresolvedExecution() ||
      this.core.explanations.hasUnresolvedExecution() ||
      this.core.repo.list('job').some((j) => j.status === 'result-unknown')
    )
      throw new DomainError(
        'RESULT_UNKNOWN',
        'A new model call was blocked because the previous analysis has not been confirmed to have ended.',
        409,
      );
    if (this.tasks.size >= QUESTION_LIMITS.pending)
      this.fail('The question queue is full. Try again shortly.');
  }
  private launch(s: QuestionSession, t: QuestionTurn, e: QuestionExecution) {
    const promise = this.run(s, t, e)
      .catch(() => {
        this.storageUnknown = true;
        // A failed metadata write must never release the UI into a retryable state.
        t.status = 'result-unknown';
        t.retryable = false;
        t.answer = null;
        t.error = 'Execution status could not be saved. Automatic retry has stopped.';
      })
      .finally(() => {
  ...
  private async run(s: QuestionSession, t: QuestionTurn, e: QuestionExecution) {
    ...
    e.repairs = Math.max(
      ...
      ...this.core.repo
        .list('questionExecution')
        .filter((x) => x.sessionId === s.id && x.turnId === t.id)
        .map((x) => x.repairs ?? 0),
    );
    ...
    const diagnose = (d: QuestionDiagnostic) => {
      e.diagnostics = [
        ...
        {
          ...d,
          itemId: d.itemId === null ? null : this.core.ids.hash(d.itemId),
          repairs: e.repairs ?? 0,
        },
      ].slice(-16);
      this.save(e);
    ...
    const validate = () => {
      if (
        this.closing ||
        this.sessions.get(s.id) !== s ||
        t.attempts !== e.attempt ||
        t.status === 'invalidated'
      )
        this.fail('The question session ended.');
      if (
        this.hasUnresolvedExecution() ||
        this.core.explanations.hasUnresolvedExecution() ||
        this.core.repo.list('job').some((j) => j.status === 'result-unknown')
        ...
        throw new DomainError(
          'RESULT_UNKNOWN',
          'Another analysis has not been confirmed to have ended.',
        );
      this.scope(s);
    };
    const phase = (status: QuestionTurn['status']) => {
      t.status = status;
      e.status = status;
      this.save(e);
      this.core.events.changed(s.workId);
    ...
    try {
      ...
      if (!context.excerpts.length) {
        t.answer = { items: [], unknowns: [questionNotice(t.text, 'empty')] };
        phase('completed');
        return;
      }
      phase('queued');
      ...
      try {
        ...
        validate();
        candidate = validateQuestionAnswer(raw, context, sources);
      } catch (error) {
        ...
        try {
          terminated = (await this.core.summary.resolve(e.remote)) === 'terminated';
        } catch {}
        if (!terminated) {
          terminationUnknown = true;
          throw new DomainError(
            'RESULT_UNKNOWN',
            'Repair stopped because the previous answer run has not been confirmed to have ended.',
          );
        }
        validate();
        e.repairs = 1;
      ...
      validate();
      const result = assessQuestionAnswer(candidate, check, t.text);
      for (const c of result.assessment.checks)
      ...
      t.assessment = {
        ...result.assessment,
        checks: result.assessment.checks.filter((c) =>
          result.answer.items.some((i) => i.id === c.itemId),
        ),
      };
      phase('completed');
    } catch (error) {
      if (error instanceof QuestionCandidateError && error !== diagnosedError)
        diagnose(error.diagnostic);
      ...
      if (!terminationUnknown)
        try {
          terminated = await this.core.summary.resolve(e.remote);
        } catch {}
      const invalid = this.sessions.get(s.id) !== s || s.invalidated || this.closing;
      t.answer = null;
      t.assessment = null;
      t.error = invalid
        ? 'The question session or connection scope ended.'
        : error instanceof DomainError || error instanceof QuestionCandidateError
          ? error.message
          : 'The model answer or its check could not be completed.';
      t.retryable =
        !invalid &&
        terminated === 'terminated' &&
        t.attempts < 2 &&
        !(error instanceof DomainError && error.code === 'CAPABILITY_UNSUPPORTED');
  ...
  evidence(workId: string, id: string, turnId: string, revisionId: string) {
    ...
    if (!t?.answer?.items.some((i) => i.evidence.some((e) => e.revisionId === revisionId)))
      this.fail('This is not a validated source for this answer.');
    const source = sources.find((s) => s.id === revisionId);
  ...
  hasUnresolvedExecution() {
    return (
      this.storageUnknown ||
      this.core.repo.list('questionExecution').some((e) => e.status === 'result-unknown')
    );
  }
  async recover() {
