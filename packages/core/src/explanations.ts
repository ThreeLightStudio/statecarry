import {
  ...
  EXPLANATION_POLICY,
  EXPLANATION_LIMITS,
  explanationPrepareSchema,
  explanationRetrySchema,
...
export class Explanations {
  ...
  private save(job: ExplanationJob) {
    this.core.repo.put('explanationJob', { ...job, updatedAt: this.now() });
  }
  available() {
    return !!this.core.summary.generateExplanation && !!this.core.summary.checkExplanation;
  }
  hasUnresolvedExecution() {
    return (
      this.storageUnknown ||
      this.core.repo.list('explanationJob').some((j) => j.status === 'result-unknown')
    );
  }
  private scope(input: ExplanationInput, current = false): SourceRevision[] {
    ...
    if (
      ...
      throw new DomainError(
        'SOURCE_UNAVAILABLE',
        'The explanation source access scope changed.',
        409,
      );
    if (
      current &&
      (c.revision !== input.connectionRevision ||
        w.linkVersion !== input.linkVersion ||
        w.latestSummaryId !== input.summaryId ||
        input.policyVersion !== EXPLANATION_POLICY ||
        this.core.ids.hash(input.analysis) !==
          this.core.ids.hash(this.core.summary.configuration()))
    )
      throw new DomainError(
        'REVISION_CONFLICT',
        'The explanation summary or settings basis changed.',
        409,
      );
  ...
  get(workId: string, id: string) {
    ...
    if (r.jobId !== this.core.ids.hash({ ...r.input, capturedAt: undefined }))
      throw new DomainError(
        'SOURCE_UNAVAILABLE',
        'The fixed explanation input identifier does not match.',
        409,
      );
    this.scope(r.input);
    return r;
  ...
  view(workId: string): ExplanationView {
    ...
    return {
      ...
      revision,
      policyVersion: EXPLANATION_POLICY,
      accessibleIds: accessible.map((r) => r.id),
      job: job
        ? {
            canRetry:
              job.status === 'failed' &&
              job.retries < EXPLANATION_LIMITS.retries &&
              job.calls + (job.candidate ? 1 : 2) <= EXPLANATION_LIMITS.calls,
            id: job.id,
            status: job.status,
            calls: job.calls,
            repairs: job.repairs,
    ...
    const input: ExplanationInput = {
            ...
            goal: {
              ...
              relations: this.core
                ...
                .map((l) => ({
                  ...
                  kind:
                    l.relation ??
                    (/병렬|parallel/i.test(l.rationale)
                      ? ('parallel' as const)
                      : /재검증|recheck/i.test(l.rationale)
                        ? ('recheck' as const)
                        : l.evidence.length
                          ? ('followup' as const)
                          : ('unclear' as const)),
                  evidenceIds: l.evidence.filter((id) => summary.sourceRevisionIds.includes(id)),
                  rationale: l.evidence.every((id) => summary.sourceRevisionIds.includes(id))
                    ? l.rationale
                    : 'Relationship evidence is outside this input; review the connection',
                })),
            },
          }
        : {}),
      ...
      limitations: [
        ...new Set([
          ...
          ...sources.flatMap((s) => s!.limitations),
          ...(!selected.selectionComplete
            ? [
                'Only some source excerpts were selected. Distinguish unselected content from absence in the records.',
              ]
            : []),
          ...(Object.keys(connection.startTurnIds).length
            ? ['Turns before your chosen starting turn are outside the input scope.']
            : []),
          ...(!summary.inputCapturedAt
            ? [
                'The earlier summary lacks collection boundary information. The presence of the original request is unknown.',
              ]
            : []),
        ]),
      ],
  ...
  retry(workId: string, jobId: string, value: unknown) {
    ...
    if (
      ...
      throw new DomainError('VALIDATION', 'No explanation retries remain.');
    this.core.repo.transaction(() => {
  ...
  tick() {
    ...
    if (
      this.closing ||
      this.task ||
      this.hasUnresolvedExecution() ||
      this.core.questions.hasUnresolvedExecution() ||
      this.core.repo.list('job').some((j) => j.status === 'result-unknown')
    )
      return;
    const pending = this.core.repo
      .list('explanationJob')
      .filter((j) => ['waiting', 'queued'].includes(j.status));
    for (const j of pending) {
      try {
        this.scope(j.input, true);
      } catch (e) {
        this.save({ ...j, status: 'superseded', error: (e as Error).message });
      }
    }
    const valid = pending
      .filter((j) => this.core.repo.get('explanationJob', j.id)?.status !== 'superseded')
      .slice(0, EXPLANATION_LIMITS.pending);
  ...
  private async run(original: ExplanationJob) {
    ...
    const update = (patch: Partial<ExplanationJob>) => {
      if (this.core.repo.get('explanationJob', job.id)?.attemptToken !== token)
    ...
    this.save(job);
    const validate = () => {
      if (this.closing || this.core.repo.get('explanationJob', job.id)?.attemptToken !== token)
        throw new DomainError('RESULT_UNKNOWN', 'The explanation run ended.');
      if (
        this.hasUnresolvedExecution() ||
        this.core.questions.hasUnresolvedExecution() ||
        this.core.repo.list('job').some((j) => j.status === 'result-unknown')
      )
        throw new DomainError('RESULT_UNKNOWN', 'The previous analysis completion is unknown.');
      if (job.id !== this.core.ids.hash({ ...job.input, capturedAt: undefined }))
        throw new DomainError(
          'SOURCE_UNAVAILABLE',
          'The explanation input storage boundary does not match.',
          409,
        );
      this.scope(job.input, true);
    };
    const phase = async (
      name: 'generating' | 'repairing' | 'checking',
      run: () => Promise<unknown>,
    ) => {
      validate();
      if (job.calls >= EXPLANATION_LIMITS.calls)
        throw new DomainError('SUMMARY_UNAVAILABLE', 'The explanation call budget is exhausted.');
      update({
        status: name,
        calls: job.calls + 1,
        remote: null,
        phases: [...job.phases, { phase: name, startedAt: this.now(), endedAt: null }],
      });
      try {
        return await run();
      } finally {
        update({
          phases: job.phases.map((p, i) =>
            i === job.phases.length - 1 ? { ...p, endedAt: this.now() } : p,
          ),
        });
      }
    };
    const remote = (meta: ExplanationJob['remote']) => {
      update({ remote: meta });
      validate();
    };
    try {
      ...
      if (!context.excerpts.length)
        throw new DomainError(
          'SOURCE_UNAVAILABLE',
          'No sources are available to prepare an explanation.',
        );
      ...
      for (;;) {
        ...
        try {
          if (!raw)
            raw = await phase(repair ? 'repairing' : 'generating', () =>
              this.core.summary.generateExplanation!(context, remote, validate, repair),
            );
          validate();
          const candidate = validateExplanation(raw, context);
          update({ candidate });
          const assessment = assessExplanation(
            candidate,
            await phase('checking', () =>
              this.core.summary.checkExplanation!(context, candidate, remote, validate),
            ),
          );
          validate();
          this.core.repo.transaction(() => {
            ...
            this.core.repo.put('explanation', result);
            update({
              status: 'ready',
              resultId: result.id,
              error: null,
          ...
          if (
            (!(e instanceof ExplanationCandidateError) &&
              (!(e instanceof DomainError) || e.code !== 'SUMMARY_UNAVAILABLE')) ||
            job.repairs >= EXPLANATION_LIMITS.repairs ||
            job.calls + 2 > EXPLANATION_LIMITS.calls
          )
            throw e;
          if ((await this.core.summary.resolve(job.remote)) !== 'terminated')
            throw new DomainError(
              'RESULT_UNKNOWN',
              'The explanation run has not been confirmed to have ended.',
            );
          validate();
          repair = { candidate: raw, reason: e.message };
          update({ candidate: null, repairs: job.repairs + 1 });
        }
      }
    } catch (e) {
      ...
      try {
        terminated = (await this.core.summary.resolve(job.remote)) === 'terminated';
      } catch {}
      let changed = false;
      try {
        this.scope(job.input, true);
      ...
      update({
        status: !terminated ? 'result-unknown' : changed ? 'superseded' : 'failed',
        error: e instanceof Error ? e.message : 'Explanation preparation failed',
      });
    }
  }
  async recover() {
    for (const job of this.core.repo
      .list('explanationJob')
      .filter((j) =>
        ['generating', 'repairing', 'checking', 'result-unknown'].includes(j.status),
      )) {
      const result = this.core.repo
        .list('explanation')
        .find((r) => r.jobId === job.id && r.attemptToken === job.attemptToken);
      if (result) {
        this.save({ ...job, status: 'ready', resultId: result.id });
        continue;
      }
      let terminated = false;
      try {
        ...
        status: !terminated
          ...
          : !valid
            ...
            : job.calls + (job.candidate ? 1 : 2) <= EXPLANATION_LIMITS.calls
              ...
              : 'failed',
        error: !terminated
          ? 'Previous explanation process completion unknown'
          : 'Recovering fixed input after the previous run ended',
      });
    }
  }
  beginClose() {
