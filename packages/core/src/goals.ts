...
export function selectedRecords(connection: Connection, threadId: string, input: SourceRevision[]) {
  let records = input;
  const turn =
    connection.startTurnIds[threadId] ?? connection.discoveryScope?.startTurnIds[threadId];
  if (turn) {
    const at = records.findIndex((s) => s.turnId === turn);
    if (at < 0)
      throw new DomainError('SOURCE_UNAVAILABLE', 'Selected start turn is missing; review scope.');
    const earlierTurns = new Set(records.slice(0, at).map((s) => s.turnId));
    records = records.slice(at).filter((s) => !earlierTurns.has(s.turnId));
  }
  const range =
    connection.recordRanges?.[threadId] ?? connection.discoveryScope?.recordRanges[threadId];
  if (!range) return records;
  const matches = (p: typeof range.start) =>
    records.flatMap((s, i) => (s.turnId === p.turnId && s.itemId === p.itemId ? [i] : []));
  const starts = matches(range.start),
    ends = range.end ? matches(range.end) : [records.length - 1];
  if (starts.length !== 1 || ends.length !== 1 || ends[0] < starts[0])
    throw new DomainError(
      'SOURCE_UNAVAILABLE',
      'Selected record boundary is missing or ambiguous; review scope. Selection was not expanded.',
    );
  return records.slice(starts[0], ends[0] + 1);
}

const goalExpression =
...
export function goalCandidates(core: StateCarry, workId: string): GoalCandidate[] {
  ...
  for (const threadId of new Set(sources.map((s) => s.threadId))) {
    ...
    const anchors = records.flatMap((s, i) =>
      s.actor === 'user' && (goalExpression.test(s.text) || separateExpression.test(s.text))
        ? [i]
        : [],
    );
    for (const [index, at] of anchors.entries()) {
      ...
      if (
        result.some(
          (c) =>
            ...
            source.text.slice(0, 1200).replace(/\s+/g, ' ').trim(),
        )
      )
        continue;
      ...
      result.push(
        old?.evidenceId === source.id
          ? old
          : {
              id,
              title: source.text.split(/(?<=[.!?。])\s/)[0].slice(0, 120),
              quote: source.text.slice(0, 1200),
              evidenceId: source.id,
              threadId,
              range: {
                start: point(source),
                ...(end === undefined ? {} : { end: point(records[end - 1]) }),
              },
              status:
                /실험|experiment/i.test(source.text) && /중단|stop|abandon/i.test(source.text)
                  ? 'dismissed'
                  : 'proposed',
            },
      );
  ...
  if (
    records.some(
      (s) =>
        s.actor === 'user' &&
        s.id !== first.id &&
        (separateExpression.test(s.text) ||
          (goalExpression.test(s.text) &&
            !/이 목표|같은 목표|same goal|same task/i.test(s.text) &&
            !relationshipEvidence([s], knownIds).length)),
    )
  )
    return [];
  return evidence;
...
): import('@statecarry/contracts').GoalRelation['kind'] {
  const text = records
