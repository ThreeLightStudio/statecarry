...
export class Continuations {
  constructor(private readonly core: StateCarry, private readonly executor: SessionExecutor) {}

  private receipt(id: string, hash: string): Receipt | null {
    const prior = this.core.repo.get('receipt', id);
    if (prior && prior.bodyHash !== hash) throw new DomainError('IDEMPOTENCY_CONFLICT', 'Request id was used with another body', 409);
    return prior;
  }
  ...
  private receiptForBody(workId: string, command: string, hash: string): Receipt | null {
    return this.core.repo.list('receipt').find(item =>
      item.workId === workId && item.command === command && item.bodyHash === hash,
    ) ?? null;
  }

  private require(id: string, workId?: string): Continuation {
    const value = this.core.repo.get('continuation', id);
    if (!value || (workId && value.workId !== workId)) throw new DomainError('NOT_FOUND', 'Continuation request not found', 404);
    return value;
  }

  private ensureCapability(mode: ContinuationTarget['mode']) {
    const capability = this.executor.capability();
    if (capability.send !== 'supported' || (mode === 'new-session' && capability.create !== 'supported')) {
      throw new DomainError('CAPABILITY_UNSUPPORTED', capability.detail || 'Continuation execution is not supported in this environment');
    }
  }
  ...
  private ensureWorkspace(work: Work) {
    ...
    if (!baseline || !workspaceSnapshotSchema.safeParse(baseline).success) {
      throw new DomainError('REVISION_CONFLICT', 'Workspace state was not captured; refresh before continuing', 409);
    }
    ...
    if (!current || baseline.status !== 'checked' || current.status !== 'checked') {
      throw new DomainError('REVISION_CONFLICT', 'Project state could not be checked; refresh before continuing', 409);
    }
    if (!Array.isArray(baseline.limitations) || !Array.isArray(current.limitations)) {
      throw new DomainError('REVISION_CONFLICT', 'Project limitations could not be confirmed; refresh before continuing', 409);
    }
    const same = baseline.cwd === current.cwd
      && baseline.root === current.root
      && baseline.branch === current.branch
      && baseline.commit === current.commit
      && baseline.dirty === current.dirty
      && (baseline.fileFingerprint ?? baseline.fingerprint ?? null) === (current.fileFingerprint ?? current.fingerprint ?? null)
      && (baseline.inventoryFingerprint ?? null) === (current.inventoryFingerprint ?? null)
      && this.core.ids.hash(((baseline.files?.length ? baseline.files : baseline.fileObservations ?? baseline.files) ?? []).map(file => ({ path: file.path, hash: file.hash, size: file.size ?? null })).sort((a, b) => a.path.localeCompare(b.path))) === this.core.ids.hash(((current.files?.length ? current.files : current.fileObservations ?? current.files) ?? []).map(file => ({ path: file.path, hash: file.hash, size: file.size ?? null })).sort((a, b) => a.path.localeCompare(b.path)))
    ...
    if (!same) throw new DomainError('REVISION_CONFLICT', 'The project changed since this brief; refresh before continuing', 409);
  }
  ...
  private ensureResumeCurrent(work: Work) {
    ...
    if (!work.resume || !work.resume.candidates.length || !resumeResultSchema.safeParse({ candidates: work.resume.candidates }).success) return;
    ...
    if (current.busy || current.stale || current.updatesAvailable) {
      throw new DomainError('REVISION_CONFLICT', 'The saved resume brief is out of date; refresh before continuing', 409);
    }
  }

  prepare(workId: string, command: Command): Continuation {
    const input = continuationPrepareSchema.parse(command.payload);
    const bodyHash = this.core.ids.hash({ action: 'continuation-prepare', workId, expectedRevision: command.expectedRevision, payload: input });
    ...
    if (work.revision !== command.expectedRevision) throw new DomainError('REVISION_CONFLICT', 'Work changed; review the current status before preparing a continuation', 409);
    ...
    if (input.payload.evidence?.some(item => {
      const source = this.core.accessibleSource(workId, item.revisionId);
      return !source || !source.text.includes(item.quote);
    })) {
      throw new DomainError('HANDOFF_EVIDENCE_INACCESSIBLE', 'Continuation evidence is outside the current work or no longer matches the record');
    }
    const link = input.targetMode === 'existing-session'
      ? this.core.links(workId).find(item => item.threadId === input.threadId && item.status === 'linked')
      : undefined;
    if (input.targetMode === 'existing-session' && !link) throw new DomainError('HANDOFF_TARGET_UNLINKED', 'The selected conversation is no longer connected');
    if (input.targetMode === 'new-session' && input.threadId != null) throw new DomainError('VALIDATION', 'A new session cannot have an existing conversation ID');
    if (input.payload.previousThreadId && !this.core.links(workId).some(item => item.threadId === input.payload.previousThreadId && item.status === 'linked')) {
      throw new DomainError('HANDOFF_TARGET_UNLINKED', 'The previous conversation is outside the connected work');
    }
    ...
    const continuation: Continuation = { id: this.core.ids.next(), workId, requestId: command.requestId, target,
      state: 'prepared', threadId: target.threadId, turnId: null, error: null, createdAt: now, updatedAt: now };
    const receipt: Receipt = { id: command.requestId, command: 'continuation-prepare', bodyHash, workId,
      committedRevision: work.revision, resultId: continuation.id, createdAt: now };
  ...
  async send(workId: string, command: Command): Promise<Receipt> {
    ...
    const bodyHash = this.core.ids.hash({ action: 'continuation-send', workId, expectedRevision: command.expectedRevision, payload: input });
    const prior = this.receipt(command.requestId, bodyHash);
    ...
    const work = this.core.work(workId);
    if (work.revision !== continuation.target.expectedRevision) throw new DomainError('REVISION_CONFLICT', 'Work changed; prepare a fresh continuation before sending', 409);
    this.ensureResumeCurrent(work);
    this.ensureWorkspace(work);
    this.ensureCapability(continuation.target.mode);
    if (!['prepared'].includes(continuation.state)) {
      if (continuation.state === 'result-unknown' || continuation.state === 'dispatching') throw new DomainError('RESULT_UNKNOWN', 'Continuation result is unknown; check its status before retrying', 409);
      throw new DomainError('VALIDATION', 'This continuation was already sent or closed');
    }
    const now = this.core.clock.now();
    const receipt: Receipt = { id: command.requestId, command: 'continuation-send', bodyHash, workId,
      committedRevision: this.core.work(workId).revision, resultId: continuation.id, createdAt: now };
    ...
    let state: Continuation['state'] = 'sent', error: string | null = null, threadId = continuation.threadId, turnId: string | null = null;
    try {
      if (continuation.target.mode === 'new-session') {
        const created = await this.executor.create({ workId, title: continuation.target.title, cwd: this.core.repo.get('connection', this.core.work(workId).projectId)!.cwd });
        threadId = created.threadId;
      }
      if (!threadId) throw new DomainError('RESULT_UNKNOWN', 'The continuation session ID was not returned', 409);
      const sent = await this.executor.send({ workId, threadId, text: this.text(continuation.target.payload) });
      ...
      state = e instanceof DomainError && ['CAPABILITY_UNSUPPORTED', 'VALIDATION'].includes(e.code) ? 'failed' : 'result-unknown';
      error = e instanceof Error ? e.message : String(e);
    }
    const current = this.require(continuation.id, workId);
    this.core.repo.put('continuation', { ...current, state, threadId, turnId, error, updatedAt: this.core.clock.now() });
    this.core.events.changed(workId);
    return receipt;
  }
  ...
  async open(workId: string, command: Command): Promise<Receipt> {
    ...
    const bodyHash = this.core.ids.hash({ action: 'continuation-open', workId, expectedRevision: command.expectedRevision, payload: input });
    const prior = this.receipt(command.requestId, bodyHash);
    if (prior) return prior;
    ...
    const continuation = this.require(input.continuationId, workId);
    const work = this.core.work(workId);
    if (work.revision !== continuation.target.expectedRevision) throw new DomainError('REVISION_CONFLICT', 'Work changed; prepare a fresh continuation before opening', 409);
    this.ensureWorkspace(work);
    if (this.core.navigator.capability().precision === 'unsupported') throw new DomainError('CAPABILITY_UNSUPPORTED', 'Opening the continuation session is not supported in this environment');
    if (!['sent', 'failed'].includes(continuation.state) || !continuation.threadId) {
      if (continuation.state === 'result-unknown' || continuation.state === 'dispatching') throw new DomainError('RESULT_UNKNOWN', 'Continuation result is unknown; check its status before opening', 409);
      throw new DomainError('VALIDATION', 'Send the continuation before opening its conversation');
    }
    const now = this.core.clock.now();
    const receipt: Receipt = { id: command.requestId, command: 'continuation-open', bodyHash, workId,
      committedRevision: this.core.work(workId).revision, resultId: continuation.id, createdAt: now };
    const transaction = this.core.repo.transaction((): { receipt: Receipt; dispatch: boolean } => {
      const again = this.receipt(command.requestId, bodyHash);
      if (again) return { receipt: again, dispatch: false };
    ...
    if (!transaction.dispatch) return transaction.receipt;
    let state: Continuation['state'] = 'opened', error: string | null = null;
    try { await this.core.navigator.open(continuation.threadId); }
    catch (e) { state = e instanceof DomainError && e.code === 'RESULT_UNKNOWN' ? 'result-unknown' : 'failed'; error = e instanceof Error ? e.message : String(e); }
    const current = this.require(continuation.id, workId);
    this.core.repo.put('continuation', { ...current, state, error, updatedAt: this.core.clock.now() });
    this.core.events.changed(workId);
    return receipt;
  }

  get(workId: string, id: string) { return this.require(id, workId); }

  recover() {
    for (const continuation of this.core.repo.list('continuation')) {
      if (!['dispatching', 'opening'].includes(continuation.state)) continue;
      this.core.repo.put('continuation', { ...continuation, state: 'result-unknown', error: 'Continuation outcome is unknown; no automatic retry', updatedAt: this.core.clock.now() });
    }
  }
}
