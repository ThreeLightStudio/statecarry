...
it('projects shared revisions with undirected edges and independent generated IDs', async () => {
  ...
  for (const item of flow.items)
    expect(graph.nodes.find((n) => n.itemIds[0] === item.id)?.label).toContain(item.summary);
  ...
  for (const node of graph.nodes.filter((n) => n.revisionId)) {
    expect(
      node.itemIds.every((id) =>
        flow.items.find((i) => i.id === id)!.sources.some((s) => s.revisionId === node.revisionId),
      ),
    ).toBe(true);
  }
});
it.each(['single', 'outside-input', 'missing', 'bad-quote', 'limited', 'unrelated'])(
  'falls back without inventing relations: %s',
  async (mode) => {
    const flow = await fixture();
    if (mode === 'single') flow.items = [flow.items.at(-1)!];
    if (mode === 'outside-input') flow.sourceRevisionIds = [];
    if (mode === 'missing')
      flow.items.forEach((i) => i.sources.forEach((s) => (s.available = false)));
    if (mode === 'bad-quote')
      flow.items.forEach((i) => i.sources.forEach((s) => (s.quoteStarts = [])));
...
it('keeps long text intact in details, encodes hostile labels, and includes execution results', async () => {
  ...
  item.typeLabel = 'Execution result';
...
it('prioritizes summaries, marks truncation at 48 code points and retains detail metadata', async () => {
  ...
  expect(graph.nodes.find((node) => node.itemIds.includes(item.id))!.label).toBe(
    'This judgment · ' + '한'.repeat(47) + '😀…',
  );
  expect(
    graph.nodes
      .filter((node) => node.revisionId)
      .every((node) => /^Shared source \d+$/.test(node.label)),
  ).toBe(true);
  expect(flow).toEqual(before);
  const exact = structuredClone(flow);
  exact.items.find((i) => i.id === item.id)!.summary = '한'.repeat(48);
  expect(
    presentFlowGraph(exact).nodes.find((node) => node.itemIds.includes(item.id))!.label,
  ).not.toContain('…');
});
it('does not truncate large collections or turn adjacency into relations', async () => {
  const flow = await fixture();
  flow.items = Array.from({ length: 25 }, (_, i) => ({
    ...structuredClone(flow.items[0]),
    id: `item${i}`,
    claimId: `claim${i}`,
  }));
  const graph = presentFlowGraph(flow);
