...
function fixture() {
  const home = mkdtempSync(join(tmpdir(), 'm4a-scope-'));
  dirs.push(home);
  ...
  const events = [
    ...
    ...['old', 'chosen', 'later'].flatMap((id) => [
      { type: 'turn_context', timestamp: '2026-09-13T00:00:00Z', payload: { turn_id: id } },
      {
        ...
        payload: {
          ...
          role: 'user',
          id: `item-${id}`,
          content: [{ type: 'input_text', text: `${id} BODY` }],
        },
      },
    ]),
    { type: 'turn_context', payload: { turn_id: 'old' } },
    {
      ...
      payload: {
        type: 'message',
        role: 'user',
        content: [{ type: 'input_text', text: 'old repeated BODY' }],
      },
    },
  ];
  writeFileSync(path, events.map((e) => JSON.stringify(e)).join('\n') + '\n');
  const request = vi.fn(async () => ({
    thread: { id: 'thread-a', path, cwd: '/fixture', name: 'Fixture' },
  }));
...
it('lists only turn metadata and never requests provider bodies before approval', async () => {
  ...
  const result = await reader.listTurns('thread-a');
...
it('first scoped collection stores and sends only the approved local turns to generation and checking', async () => {
  const { reader, request } = fixture(),
    h = harness();
  h.reader.read = reader.read.bind(reader);
  const generate = vi.spyOn(h.summary, 'generate'),
    check = vi.spyOn(h.summary, 'check');
  const id = h.core.connect({
    requestId: 'scoped',
    expectedRevision: 0,
    payload: {
      title: 'Fixture',
  ...
  for (const s of h.repo.list('source')) {
    ...
    expect(s.limitations.join(' ')).toContain(
      'missing execution details do not establish completion',
    );
  }
  expect(JSON.stringify(generate.mock.calls)).toContain(
    'Tool result and turn state coverage are partial',
  );
  expect(JSON.stringify(check.mock.calls)).toContain(
    'Tool result and turn state coverage are partial',
  );
  expect(h.core.snapshot(id).summary!.limitations.join(' ')).toContain(
    'missing execution details do not establish completion',
  );
  expect(JSON.stringify(generate.mock.calls)).not.toContain('old BODY');
  expect(JSON.stringify(check.mock.calls)).not.toContain('old BODY');
...
it('missing starting turns fail without source storage or model requests', async () => {
  ...
  await expect(reader.read('thread-a', 'bad/id')).rejects.toThrow('Invalid provider identifier');
  await h.core.close();
});
  ...
  expect(() =>
    h.core.connect({
      ...
      payload: {
        ...
        cwd: '/fixture',
        threadIds: ['thread-a'],
        startTurnIds: { other: 'chosen' },
        discover: false,
      },
    }),
  ).toThrow('outside selected');
  expect(h.repo.list('connection')).toEqual([]);
});
it('does not include earlier-turn supplements appearing after the starting turn in a reader result', async () => {
  const h = harness();
  const old = { ...source('old BODY'), turnId: 'old' },
    selected = { ...source('chosen BODY', 'thread-a', 'chosen'), turnId: 'chosen' },
    supplement = { ...source('old supplement', 'thread-a', 'supplement'), turnId: 'old' };
  h.records([old, selected, supplement]);
  const id = h.core.connect({
    requestId: 'order',
    expectedRevision: 0,
    payload: {
