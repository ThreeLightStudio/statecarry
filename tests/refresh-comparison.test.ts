...
it('publishes a fixed result after three updates without waiting for input to stop', async () => {
  ...
  h.summary.generate = async (s, remote) => {
    await gate;
    return generate(s, remote);
  };
  const run = h.core.process(id);
  for (let i = 1; i <= 3; i++) {
    now += 1000;
    h.records([source(`revision ${i}`)]);
    await h.core.collect(id);
  }
  release();
  await run;
  ...
  now += 1000;
  h.records([source('still receiving input')]);
  await h.core.collect(id);
  console.info(
    'fixed-range-comparison',
    JSON.stringify({
      collectedChangesBeforeRelease: 3,
      publishedBeforeNextInput,
      jobs: h.repo.list('job').length,
      superseded: h.repo.list('job').filter((j) => j.status === 'superseded').length,
      generationCalls: h.counts().generationCalls,
      checkCalls: h.counts().checkCalls,
      freshness: h.core.freshness(id).summary,
      processingMs: h.core.snapshot(id).summary?.processingMs ?? null,
    }),
  );
  expect(publishedBeforeNextInput).toBe(1);
