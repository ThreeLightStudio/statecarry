...
it('renders reflected and corrected ranges with distinct processing states and unknown source times', async () => {
  const h = harness(),
    id = h.connect();
  const first = { ...source(), eventAt: null };
  h.records([first]);
  await h.core.collect(id);
  await h.core.process(id);
  h.records([{ ...source('정정된 행동'), eventAt: null }]);
  await h.core.collect(id);
  const gateway: Gateway = {
    ...
    subscribe: () => () => {},
    command: async () => {
      throw new Error('No writes in render test');
    },
    receipt: async () => {
  ...
  expect(html).not.toContain('모두 반영됨');
  expect(html).toContain('thread-a / turn-a / item-a');
});
