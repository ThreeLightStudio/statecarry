...
it('keeps selection local until confirmation and sends the chosen first range or explicit all-turn range', async () => {
  ...
  const connect = vi.fn(async () => undefined),
    list = vi.fn(async () => ({ turns: [{ id: 'chosen', at: '2026-09-13' }] }));
  const input = (label: string) =>
    [...host.querySelectorAll('label')]
      .find((el) => el.textContent?.startsWith(label))!
      .querySelector('input')!;
  const change = async (el: HTMLInputElement, value: string) =>
    act(async () => {
  ...
  const click = async (text: string) =>
    act(async () =>
      [...host.querySelectorAll('button')].find((el) => el.textContent?.trim() === text)!.click(),
    );
  try {
    await act(async () =>
      root.render(
        createElement(App, {
          ...
          onAction() {},
          onConnect: connect,
          onDiscover: h.reader.discover,
          onListTurns: list,
        }),
      ),
    );
    await change(input('Project name'), 'Fixture');
    await change(input('Project folder'), '/fixture');
    await change(input('Conversation IDs continued'), 'thread-a');
    await click('Find starting turns');
    expect(list).toHaveBeenCalledWith('thread-a');
    ...
    await click('Connect selected records');
    ...
    await click('Use all accessible turns');
    await click('Connect selected records');
    expect(connect.mock.calls[1]).toEqual([
      {
        title: 'Fixture',
        cwd: '/fixture',
        threadIds: ['thread-a'],
