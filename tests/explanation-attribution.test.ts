...
import { explanationEvidenceCatalog, explanationGenerationSchema } from '../apps/server/src/adapters/explanation-prompts';
...
it('rejects user words inside a tool document and identifies the citation for repair', async () => {
  ...
  try {
    ...
    expect(() => validateExplanation(candidate, context)).toThrow(`node=${candidate.nodes[0].id}; nature=${candidate.nodes[0].nature}; actualActor=tool`);
    ...
    assessment.nodes[0] = { id: candidate.nodes[0].id, verdict: 'unsupported', reason: 'A tool document does not establish a direct user statement.' };
    expect(() => assessExplanation(candidate, assessment)).toThrow('Explanation meaning check rejected');
  } finally { await run.h.core.close(); }
});

it('constrains generated IDs and provenance at every reason depth', () => {
  const schema = explanationGenerationSchema([{ actor: 'user', fragments: [{ evidenceId: 'u' }] }, { actor: 'tool', fragments: [{ evidenceId: 't' }] }]);
  const leaf = { role: 'state', kind: 'record', nature: 'user-report', text: 'User report', uncertainty: '', condition: '', unknowns: [], evidenceIds: ['u'] };
  const input = { sections: [{ title: 'State', body: [{ ...leaf, reasons: [] as any[] }] }], unknowns: [] };
  expect(schema.safeParse(input).success).toBe(true);
  input.sections[0].body[0].evidenceIds = ['t']; expect(schema.safeParse(input).success).toBe(false);
  input.sections[0].body[0].evidenceIds = ['missing']; expect(schema.safeParse(input).success).toBe(false);
  input.sections[0].body[0].evidenceIds = ['u'];
  input.sections[0].body[0].reasons = [{ question: 'Why?', kind: 'record', uncertainty: '', evidenceIds: ['u'], node: { ...leaf, reasons: [{ question: 'Why?', kind: 'record', uncertainty: '', evidenceIds: ['u'], node: { ...leaf, evidenceIds: ['t'] } }] } }];
...
it('passes a rejected transport candidate to the single bounded repair without publishing it', async () => {
  ...
  try {
    run.prepare(); const view = await run.settled();
    expect(view.revision).not.toBeNull(); expect(view.job?.repairs).toBe(1); expect(view.job?.calls).toBe(3);
  } finally { await run.h.core.close(); }
});
