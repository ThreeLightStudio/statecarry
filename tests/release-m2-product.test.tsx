...
it('keeps connection input and ignores discovery responses for a previous folder', async () => {
  ...
  const gateway: Gateway = {
    projects: async () => [], connections: async () => [], snapshot: async id => h.core.snapshot(id),
    evidence: async id => h.core.evidence(id), discover: h.reader.discover,
    subscribe: () => () => {}, command: async () => { throw new Error('unused'); }, receipt: async () => { throw new Error('unused'); },
  };
  const controller = new Controller(gateway, { read: () => null, write() {} }, () => crypto.randomUUID());
  const host = document.createElement('div'); document.body.append(host);
  const root = createRoot(host);
  const pending: ((value: { threads: { id: string; title: string; cwd: string }[]; complete: boolean; limitations: string[] }) => void)[] = [];
  const discover = vi.fn(() => new Promise<{ threads: { id: string; title: string; cwd: string }[]; complete: boolean; limitations: string[] }>(resolve => pending.push(resolve)));
  ...
  const search = () => [...host.querySelectorAll('button')].find(el => el.textContent === 'Find conversations in this folder')!;
  try {
    await act(async () => root.render(createElement(App, { state: { ...controller.getSnapshot(), route: '#/connect' }, onAction() {}, onConnect: async () => undefined, onDiscover: discover })));
    ...
    await act(async () => pending[0]({ threads: [{ id: 'old', title: 'Wrong folder result', cwd: '/tmp/first' }], complete: true, limitations: [] }));
    ...
    await act(async () => root.unmount()); host.remove(); await h.core.close();
  }
});
...
it('separates the source-language explanation policy from question-language answers', () => {
  ...
  expect(EXPLANATION_INSTRUCTIONS + QUESTION_INSTRUCTIONS).not.toMatch(/in (?:natural )?Korean|Korean question/);
});
