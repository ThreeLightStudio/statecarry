...
async function setup() {
  ...
  const gateway: Gateway = {
    ...
    subscribe: () => () => {},
    receipt: async (key) => h.repo.get('receipt', key)!,
    command: async () => {
  ...
  try {
    const state = {
      ...
      explanation: {
        ...
        revision: null,
        latest: {
          ...t.state.explanation!.latest!,
          job: {
            ...t.state.explanation!.latest!.job!,
    ...
    const render = (value: AppViewModel) =>
      act(async () =>
        root.render(
          createElement(App, {
            state: value,
            ...
            onDiscover: t.h.reader.discover,
          }),
        ),
      );
    await render(state);
    ...
    const details = [...host.querySelectorAll('details')].find(
      (d) => d.querySelector('summary')?.textContent === 'Explanation status details',
    )!;
    expect(details.open).toBe(false);
    expect(details.textContent).toContain('Synthetic failure');
...
it('opens a selected sentence question in place and retains it across disclosure toggles', async () => {
  ...
  try {
    ...
    const state: AppViewModel = {
      ...t.state,
      question: {
        anchorType: 'explanation',
        targetKey: JSON.stringify([
          t.id,
          revision.summaryId,
          { explanationId: revision.id, nodeId: 'choice' },
        ]),
        open: true,
        input: 'Keep this unsent question',
        session: null,
        error: null,
        busy: false,
        uncertain: false,
    ...
    detail.open = false;
    await act(async () =>
      root.render(createElement(HandoffExplanation, { state: { ...state }, onAction() {} })),
    );
    ...
    expect(
      host.querySelector('[data-explanation-node="origin"] details')?.hasAttribute('open'),
    ).toBe(false);
      ...
      root.render(
        createElement(HandoffExplanation, {
          state: {
            ...
            explanation: {
              ...state.explanation!,
              revision: { ...revision, id: 'new-reading-revision' },
            },
          },
          onAction() {},
        }),
    ...
    expect(host.querySelector<HTMLTextAreaElement>('.context-questions textarea')!.value).toBe(
