...
const { renderToStaticMarkup } = requireWeb('react-dom/server') as typeof import('react-dom/server');
...
it('G1/G3: accepts a non-candidate goal as current input without borrowing another connection or changing ranges', async () => {
  ...
  h.core.updateConnection(c.id, h.command(h.project, { title: c.title, cwd: c.cwd, threadIds: ['session-a'], discover: false, startTurnIds: {}, recordRanges: { 'session-a': { start: { turnId: 'turn-a', itemId: 'ship' }, end: { turnId: 'turn-a', itemId: 'ship' } } } }));
  ...
  expect(h.core.work(h.project).goal).toMatchObject({ origin: 'user-input', text: 'Ship the billing page' });
...
it('G2: reconfirming a goal withdraws previous explanations even when sources stay accessible', async () => {
  const h = contextHarness();
  h.summary.generateExplanation = async ctx => ({ sections: [{ id: 'body', title: 'Current', bodyIds: ['state'] }], nodes: [{ id: 'state', role: 'state', kind: 'interpretation', nature: 'agent-interpretation', text: 'Selected goal state', condition: '', uncertainty: 'Selected input only', unknowns: [], evidence: [{ revisionId: ctx.excerpts[0].revisionId, start: 0, quote: ctx.excerpts[0].text }] }], links: [], unknowns: [] });
  ...
  await h.core.process(id); await h.core.explanations.settled();
...
it('G2: missing goal evidence leaves a recovery entry point and reconfirmation unblocks analysis', async () => {
  ...
  h.records.set('session-a', r.A.slice(1)); await h.core.collect(id);
...
it('legacy goal entry opens Resume without creating another work', () => {
  const html = renderToStaticMarkup(createElement(GoalChoices, { state: { goalCandidates: [] } as unknown as AppViewModel, onAction: () => {} }));
...
it('V1/V3/V4: restores the chosen report after range expansion, adopts verified result, withdraws D and prunes handoff selection', async () => {
  ...
  h.summary.generateExplanation = async ctx => {
    const e = ctx.excerpts.find(e => e.itemId === 'D3b') ?? ctx.excerpts.find(e => e.itemId === 'D3a') ?? ctx.excerpts.find(e => e.itemId === 'B3')!;
    return { sections: [{ id: 'body', title: 'Scope state', bodyIds: ['state'] }], nodes: [{ id: 'state', role: 'state', kind: 'interpretation', nature: 'agent-interpretation', text: e.itemId, condition: '', uncertainty: 'Selected input', unknowns: [], evidence: [{ revisionId: e.revisionId, start: e.start, quote: e.text }] }], links: [], unknowns: [] };
  };
  h.summary.checkExplanation = async (_, candidate) => fixtureAssessment(candidate);
  await h.core.collect(h.project);
  const candidate = h.core.goalCandidates(h.project).find(c => c.evidenceId === r.A[0].id)!;
  const id = h.core.chooseGoal(h.project, h.command(h.project, { candidateId: candidate.id, action: 'confirm' })).workId;
  const run = async () => { await h.core.collect(id); await h.core.process(id); await h.core.explanations.settled(); };
  await run();
  const memory = new Map<string, import('@statecarry/presentation').LocalWorkState>();
  const controller = new Controller({ projects: async () => h.core.listProjects(), connections: async () => h.repo.list('connection'), snapshot: async id => h.core.snapshot(id), evidence: async (sid, wid) => h.core.evidence(sid, wid), discover: h.reader.discover, subscribe: () => () => {}, command: async () => { throw new Error('Unexpected mutation'); }, receipt: async () => { throw new Error('Unexpected receipt'); }, explanation: async <T>(path: string) => h.core.explanations.get(id, path.split('/').at(-1)!) as T }, { read: id => memory.get(id) ?? null, write: (id, value) => { memory.set(id, value); } }, h.core.ids.next);
  await controller.start(`#/work/${id}`);
  const scope = (end?: string) => { const c = h.repo.get('connection', h.core.work(id).projectId)!; h.core.updateConnection(c.id, h.command(id, { title: c.title, cwd: c.cwd, threadIds: end ? ['session-a', 'session-b', 'session-c', 'session-d'] : ['session-a', 'session-b', 'session-c'], startTurnIds: {}, discover: false, recordRanges: { 'session-a': c.recordRanges!['session-a'], ...(end ? { 'session-d': { start: { turnId: 'turn-a', itemId: 'D2' }, end: { turnId: 'turn-a', itemId: end } } } : {}) } })); };
  scope('D3a'); await run(); await controller.refresh();
  ...
  scope('D3b'); await controller.refresh();
  expect(memory.get(id)?.readingExplanationId).toBe(reportId);
  await h.core.collect(id); await controller.refresh();
  expect(controller.getSnapshot().explanation?.revision?.id).toBe(reportId);
  await h.core.process(id); await h.core.explanations.settled(); await controller.refresh();
  expect(controller.getSnapshot().explanation?.newAvailable).toBe(true);
  await controller.action({ type: 'explanationAdopt' });
...
it('G2: changed candidate text is re-proposed instead of reusing obsolete confirmed evidence', async () => {
  ...
  const changed = { ...source('목표를 바꿔서 청구서 화면만 확인해줘.', 'session-a', 'A1'), actor: 'user' as const, kind: 'userMessage' };
  h.records.set('session-a', [changed, ...r.A.slice(1)]); await h.core.collect(h.project);
...
it('G2/G3: a late summary from before reconfirmation cannot publish under the revised goal', async () => {
  const h = contextHarness(); await h.core.collect(h.project);
  h.core.describeGoal(h.project, h.command(h.project, { text: 'Understand the original CSV checks' }));
  let release!: () => void, entered!: () => void;
  const gate = new Promise<void>(resolve => { release = resolve; });
  const started = new Promise<void>(resolve => { entered = resolve; });
  const generate = h.summary.generate;
  h.summary.generate = async (...args) => { entered(); await gate; return generate(...args); };
  const pending = h.core.process(h.project); await started;
  h.core.describeGoal(h.project, h.command(h.project, { text: 'Understand only the error notification checks' }));
  release(); await pending;
  expect(h.core.snapshot(h.project).summary).toBeNull();
  expect(h.repo.list('job').some(j => j.status === 'superseded')).toBe(true);
  h.summary.generate = generate; await h.core.process(h.project);
  expect(h.core.snapshot(h.project).summary?.inputVersion).toBe(h.core.work(h.project).inputVersion);
  await h.core.close();
});
...
it('G2/G3: current user goal and unchanged closed ranges survive SQLite reopen', async () => {
  ...
  try {
    await h.core.collect(h.project);
    const c = h.repo.get('connection', h.core.work(h.project).projectId)!;
    h.core.updateConnection(c.id, h.command(h.project, { title: c.title, cwd: c.cwd, threadIds: ['session-d'], startTurnIds: {}, discover: false, recordRanges: { 'session-d': { start: { turnId: 'turn-a', itemId: 'D2' }, end: { turnId: 'turn-a', itemId: 'D3a' } } } }));
    await h.core.collect(h.project);
    h.core.describeGoal(h.project, h.command(h.project, { text: 'Understand the reported Chrome retest' }));
    const expected = h.core.work(h.project).goal;
    await h.core.close(); repo.close(); repo = new SQLiteRepository(directory);
...
it('M1/M5: every summary claim, including missing and rejected claims, requires its own assessment', async () => {
  ...
  const c = candidate(r.A[0]); c.claims[3] = { ...c.claims[3], text: null, evidence: [], missing: 'not-in-record' };
  const catalog = summaryCheckCatalog(c);
  const checks = Object.fromEntries(c.claims.map(claim => [claim.id, { verdict: claim.text ? 'unsupported' : 'supported', reason: 'An assessed decision, never an omitted claim' }]));
  expect(catalog.decode({ checks }).checks).toHaveLength(c.claims.length);
  delete checks.next; expect(() => catalog.decode({ checks })).toThrow();
  expect(() => catalog.decode({ checks: [{ claimId: 'purpose', verdict: 'supported', reason: 'Only one' }] })).toThrow();
});

it('V4: selecting a saved closed question reopens its preserved input after restoration', async () => {
  const { QuestionController, emptyQuestionView } = await import('../packages/presentation/src/questions');
  let view = emptyQuestionView();
  const controller = new QuestionController({} as import('@statecarry/presentation').Gateway, () => 'request', next => { view = next; });
  ...
  controller.reset(false); controller.restore(saved);
  ...
  expect(view).toMatchObject({ open: true, input: 'Keep this earlier question', targetKey: earlier.key });
...
it('M1: semantic repair recomposes an incomplete integrated judgment instead of appending a duplicate state', async () => {
  ...
  const { explanationRepairCatalog } = await import('../apps/server/src/adapters/explanation-prompts');
  const t = await explanationHarness(); t.prepare(); const view = await t.settled();
  const context = contextFromInput(view.revision!.input);
  context.input.goal = { intent: { text: 'Understand the recorded work', origin: 'user-input', confirmedAt: '2026-09-14T00:00:00Z' }, relations: [], recordRanges: {} };
  const e = context.excerpts[0];
  const candidate = { sections: [{ id: 'current', title: 'Current', bodyIds: ['state'] }], nodes: [{ id: 'state', role: 'state' as const, kind: 'interpretation' as const, nature: 'agent-interpretation' as const, text: 'Current bounded judgment', uncertainty: 'Selected input', condition: '', unknowns: [], evidence: [{ revisionId: e.revisionId, start: e.start, quote: e.text.slice(0, 100) }] }], links: [], unknowns: [] };
  ...
  const replacement = { role: 'state', kind: 'interpretation', nature: 'agent-interpretation', text: 'Recomposed bounded judgment', uncertainty: 'Selected input', condition: '', unknowns: [], evidenceIds: ['e0-0'] };
  ...
  expect(() => catalog.decode({ ...valid, nodes: { state: { ...replacement, evidenceIds: ['invented-citation'] } } })).toThrow();
  expect(() => catalog.decode({ ...valid, nodes: {} })).toThrow();
  expect(() => catalog.decode({ ...valid, additions: [{ beforeSectionId: 'end', section: { title: 'Another current', body: [{ ...replacement, reasons: [] }] } }] })).toThrow();
  expect(() => catalog.decode({ ...valid, additions: [{ beforeSectionId: 'current', section: { title: 'Another current', body: [{ ...replacement, reasons: [] }] } }] })).toThrow();
