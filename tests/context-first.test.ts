...
it('UX1–UX2: proposes distinct goals, persists choices, and includes explicit followup and parallel sources', async () => {
  ...
  expect(h.core.sources(chosen.workId).map((s) => s.itemId)).toEqual(
    [...r.A, ...r.B, ...r.C].map((s) => s.itemId),
  );
  expect(h.core.goalCandidates(h.project).find((c) => c.id === candidate.id)?.status).toBe(
    'confirmed',
  );
...
it('UX3–UX4 publishes an initial explanation and schedules another after dedicated followup discovery', async () => {
  ...
  h.summary.generateExplanation = async (context) => {
    ...
    return {
      sections: [{ id: 'body', title: 'Current goal', bodyIds: ['state'] }],
      nodes: [
        {
          id: 'state',
          role: 'state',
          ...
          text: '선택한 CSV 목표의 현재 조건을 확인합니다.',
          uncertainty: '합성 화면 및 상태 전이 검사 대역입니다.',
          condition: '',
          unknowns: [],
...
it('C07/C08: keeps a missing-session condition separate from a scope withdrawal and preserves stored sources', async () => {
  ...
  h.records.delete('session-c');
  await h.core.collect(id);
  expect(h.core.snapshot(id).checkpoints.find((cp) => cp.threadId === 'session-c')?.status).toBe(
    'failed',
  );
...
it('keeps material constraints and failures in long input while explicitly marking excerpt selection partial', async () => {
  ...
  const sources = [
    ...
    ...Array.from({ length: 80 }, (_, i) => ({
      ...source('routine log '.repeat(400), 'session-a', `long-${i}`),
...
it('preserves the original discovery boundaries and persists goal choices without migrating existing entities', async () => {
  ...
  try {
    const h = contextHarness(repo),
      original = h.core.repo.get('connection', h.core.work(h.project).projectId)!;
    h.core.updateConnection(
      original.id,
      h.command(h.project, {
        title: original.title,
        cwd: original.cwd,
        threadIds: original.threadIds,
        startTurnIds: {},
        discover: true,
        recordRanges: {
          'session-d': {
            start: { turnId: 'turn-a', itemId: 'D2' },
            end: { turnId: 'turn-a', itemId: 'D3b' },
          },
        },
      }),
    );
    ...
    expect(
      repo.get('connection', h.core.work(id).projectId)?.discoveryScope?.recordRanges['session-d']
        .end?.itemId,
    ).toBe('D3b');
    expect(repo.get('connection', original.id)?.recordRanges?.['session-d'].start.itemId).toBe(
      'D2',
    );
  } finally {
    repo.close();
...
it('UX2/UX4/UX5: closes a mixed session at an item, prevents cross-goal evidence access, fails closed if boundaries disappear', async () => {
  ...
  h.core.updateConnection(
    ...
    h.command(id, {
      title: c.title,
      cwd: c.cwd,
      threadIds: [...c.threadIds, 'session-d'],
      startTurnIds: c.startTurnIds,
      discover: true,
      recordRanges: {
        ...c.recordRanges,
        'session-d': {
  ...
  h.records.set(
    'session-d',
    r.D.filter((s) => s.itemId !== 'D3b'),
  );
  await h.core.collect(id);
  expect(h.core.sources(id).some((s) => s.threadId === 'session-d')).toBe(false);
  expect(h.core.snapshot(id).checkpoints.find((cp) => cp.threadId === 'session-d')?.status).toBe(
    'failed',
  );
});
