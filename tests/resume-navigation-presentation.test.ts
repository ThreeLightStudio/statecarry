...
const work = (navigation: NonNullable<ResumeWork['navigation']>): ResumeWork => ({
  ...
  candidates: [
    {
      key: 'candidate',
      goal: 'Check navigation',
      currentState: 'The navigation check remains open.',
      status: 'active',
      reason: 'The check is open.',
      nextAction: 'Run the navigation check',
      actionSource: 'recorded',
      doneWhen: 'The navigation result is recorded',
      threadId: 'thread-nav',
      prerequisites: [],
      evidence: [{ revisionId: 'record-nav', quote: 'The check is open.' }],
    },
  ],
});
...
it('only presents a conversation link when navigation capability is verified', () => {
  const invalid = presentResumeWork(
    work({
      ...
      detail: 'Route verification failed.',
    }),
  );
