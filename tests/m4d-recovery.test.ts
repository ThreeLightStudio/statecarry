...
it('repairs unsafe unknowns without permitting supported assertions or evidence to be rewritten', async () => {
  ...
  try {
    ...
    candidate.nodes[0].unknowns = [
      { text: 'No analysis ever happened', impact: 'Start new analysis', cause: 'not-selected' },
    ];
    ...
    const patch = {
      ...
      additions: [],
      unknowns: [],
      nodeUnknowns: Object.fromEntries(candidate.nodes.map((n) => [n.id, []])),
    };
    ...
    expect(
      catalog.schema.safeParse({ ...patch, nodes: { [candidate.nodes[0].id]: candidate.nodes[0] } })
        .success,
    ).toBe(false);
    ...
    expect(
      explanationRepairCatalog(context, candidate, fixtureAssessment(candidate)).schema.safeParse(
        patch,
      ).success,
    ).toBe(false);
  } finally {
...
it('keeps the historical anchor and later completion/blocker reports distinguishable within the same fixed context', () => {
  const sources = [
    [
      'user',
      'Build an evaluation corpus and replay runner; validate consumption before comparing results.',
    ],
    ['tool', 'Unrelated build output '.repeat(3000)],
    [
      'agent',
      'The corpus and replay runner are now implemented. Consumption remains unverified; comparisons are blocked.',
    ],
  ].map(([actor, text], i) => ({
    id: `r${i}`,
    actor,
  ...
  const context = selectQuestionContext(
    { turns: [] } as unknown as QuestionSession,
    'Why is this action necessary?',
    { text: sources[0].text, evidence: [{ revisionId: 'r0', quote: sources[0].text }] },
    sources,
  );
