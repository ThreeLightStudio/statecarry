...
const { renderToStaticMarkup } = webRequire('react-dom/server'),
  { createElement } = webRequire('react');
...
it('renders usable activation guidance and keeps inaccessible selections explicitly removable', async () => {
  ...
  const gateway: Gateway = {
    ...
    subscribe: () => () => {},
    command: async (_path, command) =>
      h.core.prepareHandoff(id, command.expectedRevision, command.payload),
    receipt: async () => {
      throw new Error('none');
    },
  };
  const controller = new Controller(gateway, { read: () => null, write() {} }, identity.next);
  ...
  let html = renderToStaticMarkup(
    createElement(HandoffPanel, { state: controller.getSnapshot(), onAction() {} }),
  );
  for (const text of [
    '기록 A',
    'Work records',
    'Set up opening in Codex',
    '--verify-navigation',
    'thread-a',
    'Codex conversation',
  ])
    expect(html).toContain(text);
  expect(html).not.toContain('--navigation-evidence');
  expect(html).not.toContain('--register-navigation-evidence');
  ...
  expect(html).toContain('Outside the current access scope');
  expect(html).toContain('Remove this evidence');
  expect(controller.getSnapshot().local.evidenceIds).toContain('inaccessible');
  const state = controller.getSnapshot();
  html = renderToStaticMarkup(
    createElement(HandoffPanel, {
      state: { ...state, local: { ...state.local, basisSummaryId: 'previous-summary' } },
      onAction() {},
    }),
  );
