...
describe('scope and target changes', () => {
  it('changes ranges atomically, preserves source and rejects stale repeat bodies', async () => {
    ...
    const command = h.command(id, {
      ...
      startTurnIds: { 'thread-a': 'later-turn' },
      discover: false,
    });
    const receipt = h.core.updateConnection(c.id, command);
    expect(h.core.sources(id)).toEqual([]);
    expect(h.repo.get('source', source().id)).not.toBeNull();
    expect(h.core.updateConnection(c.id, command)).toEqual(receipt);
    expect(() =>
      h.core.updateConnection(c.id, {
        ...command,
        payload: { ...command.payload, discover: true },
      }),
  ...
  it('does not apply a read started before a range update', async () => {
    ...
    const pending = h.core.collect(id),
      c = h.core.snapshot(id).connection;
    ...
    await pending;
    expect(h.core.sources(id)).toEqual([]);
    expect(h.repo.get('source', source().id)).toBeNull();
  });
  it('rejects foreign source items before any checkpoint commit', async () => {
    const h = harness(),
      id = h.connect();
    h.reader.read = async () => ({ ...read([source()]), revisions: [source('wrong', 'thread-b')] });
    await h.core.collect(id);
    expect(h.core.sources(id)).toEqual([]);
  ...
  it('makes discovery failure visible while preserving last successful scope', async () => {
    const h = harness(),
      id = h.connect(),
      c = { ...h.core.snapshot(id).connection, discover: true };
    h.repo.put('connection', c);
    await h.core.discover(c);
    const good = h.core.snapshot(id).connection;
    h.reader.discover = async () => {
      throw new Error('disconnected discovery');
    };
    await h.core.discover(good);
    const state = h.core.snapshot(id).connection.discovery!;
    expect(state.status).toBe('failed');
    expect(state.successfulAt).toBe(good.discovery!.successfulAt);
    expect(state.limitations).toContain('disconnected discovery');
  });
  it('records missing and replaced source without deleting immutable history', async () => {
    const h = harness(),
      id = h.connect();
    h.records([source(), source('old extra', 'thread-a', 'extra')]);
    await h.core.collect(id);
    h.reader.read = async () => ({ ...read([source()]), generation: 'replacement' });
    await h.core.collect(id);
    const s = h.core.snapshot(id);
    expect(s.checkpoints[0].status).toBe('partial');
    expect(s.checkpoints[0].limitations.join(' ')).toContain('absent');
    expect(s.checkpoints[0].limitations.join(' ')).toContain('generation changed');
    expect(h.repo.list('source')).toHaveLength(2);
  });
});
...
describe('actual loopback HTTP contract with simulated provider', () => {
  it('guards origin/host/schema, has receipts, rejects work execution', async () => {
    const h = harness(),
      ...
      server = createHttpServer(h.core, events, '/nonexistent', 4310);
    ...
    try {
      expect(
        (await call('/projects', 'GET', undefined, { Origin: 'https://foreign.test' })).status,
      ).toBe(403);
