...
function work(patch: Partial<ResumeWork> = {}): ResumeWork {
  return {
    workId: 'work-a', title: 'Export fix', cwd: '/project', version: 'v1',
    goalText: null, goalOrigin: 'inferred', sessionCount: 1,
    updatesAvailable: false, busy: false, error: null, stale: false,
    generatedAt: '2026-09-15T00:00:00.000Z', correctedKeys: [], dismissedKeys: [],
...
describe('resume presentation status', () => {
  it.each([
    ...
    ['unavailable before the first analysis', work({ generatedAt: null, candidates: [] }), 'unavailable'],
  ...
  it('keeps the state decision and blocked actions in the UI-independent view model', () => {
    ...
    expect(view.limitations).toContain('New connected records arrived after this brief was captured.');
  });

  it('does not expose a new-session target while the work is limited', () => {
    const view = presentResumeWork(work({
      session: { create: 'supported', send: 'supported', detail: 'Sessions are available.', verifiedAt: '2026-09-15T00:00:00Z' },
  ...
  it('does not expose a new-session target for a waiting candidate', () => {
    const value = work({
      session: { create: 'supported', send: 'supported', detail: 'Sessions are available.', verifiedAt: '2026-09-15T00:00:00Z' },
      candidates: [{ ...candidate, status: 'waiting', nextAction: null, doneWhen: null, actionSource: null }],
    });
  ...
  it('keeps a retained brief in the limited state after a check error', () => {
    const view = presentResumeWork(work({
      state: 'limited', error: 'A connected record could not be read.',
...
describe('manual continuation', () => {
  ...
  it('does not invent a continuation when no next action or completion condition exists', () => {
    const current = work({ candidates: [{ ...candidate, nextAction: null, doneWhen: null, actionSource: null }] });
    const view = presentResumeWork(current).selected!;
  ...
  it('does not resurrect stale action text from a non-active candidate', () => {
    const current = work({ candidates: [{ ...candidate, status: 'done', nextAction: 'Run it again', doneWhen: 'It passes' }] });
    const view = presentResumeWork(current).selected!;
    expect(manualContinuationText(view, current)).toBeNull();
  });
});
...
describe('coordination presentation', () => {
  it('keeps an unconfirmed role visible without treating it as a selected conversation', () => {
    const result = presentCoordination(work({ coordinationChoices: [{ threadId: 'thread-a', title: 'Progress notes' }], coordination: {
      state: 'unconfirmed', threadId: 'thread-a', title: 'Progress notes',
      detail: 'The role of this conversation has not been confirmed.', evidence: [],
    } }));
