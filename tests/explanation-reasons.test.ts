...
it.each(['background', 'goal', 'progress', 'premise'])(
  'forbids reasons on a root %s while retaining its content',
  (role) => {
    const input = {
      sections: [{ title: '기록', body: [{ ...leaf, role, reasons: [reason] }] }],
      unknowns: [],
    };
    ...
    expect(
      schema.safeParse({
        ...input,
        sections: [{ title: '기록', body: [{ ...leaf, role, reasons: [] }] }],
      }).success,
    ).toBe(true);
  },
);

it.each(['choice', 'state', 'action', 'followup'])(
  'preserves two reason levels below a root %s',
  (role) => {
    const input = {
      sections: [{ title: '기록', body: [{ ...leaf, role, reasons: [reason] }] }],
      unknowns: [],
