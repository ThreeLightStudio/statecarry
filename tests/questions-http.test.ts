...
it('HTTP binds work/session/answer and rejects arbitrary revisions and hostile origin', async () => {
  const { h, id } = await questionHarness(), port = 4397;
  ...
  try {
    const input = { requestId: 'create', summaryId: h.core.work(id).latestSummaryId, claimId: 'next' };
    ...
    const result = await (await fetch(`${base}/${s.id}`)).json(), t = result.turns[0];
    expect((await fetch(`${base}/${s.id}/turns/${t.id}/evidence/${questionRecords[0].id}`)).status).toBe(200);
    expect((await fetch(`${base}/${s.id}/turns/${t.id}/evidence/${questionRecords[4].id}`)).status).toBe(409);
