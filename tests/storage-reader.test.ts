...
const dir = () => {
  const d = mkdtempSync(join(tmpdir(), 'statecarry-test-'));
  dirs.push(d);
  return d;
};
const open = (d = dir()) => {
  const r = new SQLiteRepository(d);
  repos.push(r);
  return r;
};
afterEach(() => {
...
describe('actual SQLite storage', () => {
  ...
  it('reopens work, valid summary, overlay, draft and receipt with the same identities', async () => {
    ...
    const receipt = h.core.mutate(id, 'drafts', command);
    r.close();
    repos.splice(repos.indexOf(r), 1);
    const second = open(d),
      hh = harness(second),
      restored = hh.core.snapshot(id);
    expect(restored.summary?.id).toBe(s.summary!.id);
    expect(restored.draft?.text).toBe('persistent draft');
  ...
  it('refuses a second writer and invalid databases', () => {
    const d = dir();
    open(d);
    expect(() => new SQLiteRepository(d)).toThrow('Another writer');
    const bad = dir();
    writeFileSync(join(bad, 'statecarry.sqlite'), 'not a SQLite database');
...
describe('collection normalization and coverage', () => {
  it('retains raw original IDs and line positions and excludes reasoning', () => {
    const content =
      [
        ...
        {
          type: 'response_item',
          timestamp: '2026-09-08T13:35:01Z',
          payload: {
            type: 'message',
            role: 'user',
            id: 'raw-id',
            content: [{ type: 'input_text', text: '정정 원문' }],
          },
        },
        ...
        .join('\n') + '\n';
    const result = parseRollout(content);
  ...
  it('does not parse a partially written line and reports damaged lines', () => {
    ...
    expect(result.limitations.some((x) => /Incomplete.*JSONL.*deferred/.test(x))).toBe(true);
    expect(result.limitations.some((x) => x.includes('Invalid JSONL'))).toBe(true);
  });
  it('preserves nested tool errors independently from outer completion', () => {
    const record = sourceText({
      type: 'mcpToolCall',
      tool: 'download',
      status: 'completed',
      result: { isError: true, content: 'expired' },
    });
    expect(record?.text).toContain('isError');
    expect(record?.text).toContain('expired');
    expect(record?.actor).toBe('tool');
  });
  it('fails closed when the provider returns another thread', async () => {
    const r = new CodexReader({
      request: async () => ({ thread: { id: 'thread-b', turns: [] } }),
      close: async () => {},
    });
  ...
  it('tracks same-turn text and turn status changes in immutable version IDs', async () => {
    ...
    const r = new CodexReader({
      request: async () => ({ thread: { id: 'thread-a', turns: [turn] } }),
      close: async () => {},
    });
    const before = await r.read('thread-a');
    item.text = 'corrected result';
    turn.status = 'completed';
    const after = await r.read('thread-a');
    expect(after.revisions[0].key).toBe(before.revisions[0].key);
    expect(after.revisions[0].id).not.toBe(before.revisions[0].id);
    expect(after.revisions[1].id).not.toBe(before.revisions[1].id);
  });
  it('bounds paging, keeps the original filter and reports a repeated cursor', async () => {
    ...
    const r = new CodexReader({
      request: async (_m, p) => {
        calls.push(p);
        return { data: [{ id: 'thread-a', cwd: '/test', name: 'A' }], nextCursor: 'again' };
      },
      close: async () => {},
    });
    const result = await r.discover('/test');
  ...
  it('labels narrower tool coverage and removes credential-like analysis input', () => {
    const tool = {
      ...source('x'.repeat(10000) + ' nested error: failed ' + 'y'.repeat(10000)),
      actor: 'tool' as const,
    };
    const secret = source('Bearer ' + 'a'.repeat(40));
    const input = JSON.stringify(analysisChunks([tool, secret]));
    expect(input).toContain('UNANALYZED GAP');
    expect(input).toContain('nested error');
    expect(input).toContain('CREDENTIAL OMITTED');
    expect(input).not.toContain('a'.repeat(40));
