...
it('removes active SVG, external resources and event handlers', () => {
  const clean = sanitizeFlowSvg(
    '<svg xmlns="http://www.w3.org/2000/svg" onload="alert(1)"><script>alert(1)</script><foreignObject><div>bad</div></foreignObject><a href="https://example.com"><text>link</text></a><image href="https://example.com/a"/><style>@import "https://example.com/x";</style><rect onclick="alert(1)" fill="url(https://example.com/x)"/><text>안전한 라벨</text></svg>',
  );
...
it('decodes renderer entity text without interpreting HTML and removes embedded CSS for CSP', () => {
  const clean = sanitizeFlowSvg(
    '<svg xmlns="http://www.w3.org/2000/svg" id="fixture-svg"><style>.node{fill:red}</style><text>&amp;#60;script&amp;#62; &amp;#34;괄호&amp;#34;</text></svg>',
  );
