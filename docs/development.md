...
Stage 2A installs Oxfmt/Oxlint and the shared verification contract described above. Its local acceptance checks passed from a source-only export with a frozen install: formatting, lint, TypeScript, architecture boundaries, 384 tests and the production build all completed. CI uses macOS 15 on Apple Silicon with Node `24.14.1`, pnpm `10.33.2` and RTK `0.28.2`, then performs a frozen install and runs the same `verify` command. The workflow configuration was checked locally; a remote CI run has not been performed as part of this stage. Turborepo remains stage 2B. Electrobun installation is a later delivery milestone. See [the implementation and acceptance plan](tooling-and-desktop-plan.md).
