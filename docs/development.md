...
The browser entry point constructs the real `Root` with its controller, Resume gateway and `LocalResumeMemory`. Resume edits use the `statecarry.resume.v1.` browser-storage namespace; legacy detail edits keep their existing namespace. Browser storage contains user drafts/reading preferences only. Runtime rows may be retained in tab memory but must remain non-actionable until the server revalidates them after remount/reconnection. Keep generation out of route-entry and connection-event callbacks. Synthetic route integration tests exercise details, restore, memory recreation and failed transport; they do not establish human work resumption.

Resume state regressions live in `tests/resume-validity.test.ts`, `tests/resume-work-state-ui.test.tsx` and `tests/continuation-validity.test.ts`. They cover work-owned edits, delayed responses, scope/evidence invalidation, same-scope read outages and handoff dispatch checks. Keep the distinction between an informational inspection note and an actual action blocker. Do not remove the same-scope read-retention tests to make access checks pass, or bypass evidence checks to retain a brief. UI draft tests cover a mounted screen; they do not establish persistence across remounts or browser restarts.
