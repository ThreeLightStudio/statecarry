...
Stage 2C source settings are applied locally: MIT with `Copyright (c) 2026 ThreeLight Studio`, SPDX declarations on all six workspace manifests, and `origin` connected to the owner-created `ThreeLightStudio/statecarry`. Publication steps and dependency findings are documented in [public release preparation](public-release.md) and [third-party attribution](third-party.md). Source push and remote CI remain stage 3 work; compiled-artifact notices remain a separate delivery check.
