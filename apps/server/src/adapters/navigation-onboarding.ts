...
export async function verifyNavigationArrival(options: {
  ...
  try {
    ...
    return {
      ...
      path,
      capability: inspectNavigationEvidence(options.dataDir, options.platform),
    };
  } catch (error) {
  ...
  const answer = await options.confirm(
    `The OS accepted the open request. Inspect Codex and independently compare the intended conversation's title and content. A blank screen or a different conversation is NOT success. Only if the exact conversation arrived, type "${expected}"; otherwise press Enter: `,
  );
  ...
  const evidence = navigationEvidenceSchema.parse({
    ...
    source: {
      ...
      section: 'independent-user-arrival-confirmation',
    },
    recordedAt: at,
    observedAt: at,
    environment: {
    ...
    limitations: [
      'User-confirmed title/content match, not automated foreground ID verification.',
      'App version not captured. One target does not prove every target or future version works.',
      'No message-position navigation or message submission verified.',
    ],
  });
