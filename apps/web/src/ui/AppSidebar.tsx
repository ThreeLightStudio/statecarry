  ...
  return (
    <aside className="sidebar" aria-label="StateCarry navigation">
      ...
      <nav aria-label="Projects">
        <a
          className={
            !activeWorkId && !activeDetails && !activeConnect ? 'nav-link active' : 'nav-link'
          }
          href="#/resume"
        >
          Resume
        ...
        {(works ?? []).map((work) => (
          <a
            key={work.workId}
            className={
              activeWorkId === work.workId && !activeDetails ? 'nav-link active' : 'nav-link'
            }
            href={`#/resume/${encodeURIComponent(work.workId)}`}
          >
            {work.title || 'Connected work'}
            <small className="sidebar-work-meta">
              {work.sessionCount} connected conversation{work.sessionCount === 1 ? '' : 's'}
            </small>
          </a>
        ))}
        {activeWorkId ? (
          <a
            className={activeDetails ? 'nav-link active' : 'nav-link'}
            href={`#/details/${encodeURIComponent(activeWorkId)}`}
          >
            Review details
          </a>
        ) : null}
      ...
      <div className="sidebar-footer">
        ...
        Codex records on this device
        <p>StateCarry prepares a brief. Continue in Codex when you are ready.</p>
      </div>
    </aside>
  );
}
