...
mermaid.initialize({ startOnLoad: false, securityLevel: 'strict', theme: 'neutral', htmlLabels: false, fontFamily: 'sans-serif',
  flowchart: { htmlLabels: false, useMaxWidth: false }, maxTextSize: 500_000 });
let serial = 0;
...
export function sanitizeFlowSvg(svg: string): string {
  const clean = DOMPurify.sanitize(svg, { USE_PROFILES: { svg: true, svgFilters: true },
    FORBID_TAGS: ['foreignObject', 'a', 'image', 'script'], FORBID_ATTR: ['href', 'xlink:href'] });
  const doc = new DOMParser().parseFromString(clean, 'image/svg+xml');
  if (doc.querySelector('parsererror') || doc.documentElement.localName !== 'svg') throw new Error('Invalid SVG');
  for (const element of doc.querySelectorAll('*')) {
    for (const attr of [...element.attributes]) {
      if (/^on/i.test(attr.name) || /url\(\s*["']?(?!#)/i.test(attr.value)) element.removeAttribute(attr.name);
    }
  }
  ...
  while (walker.nextNode()) walker.currentNode.textContent = walker.currentNode.textContent!.replace(/&#(\d+);/g,
    (match, digits) => { const code = Number(digits); return code > 0 && code <= 0x10ffff ? String.fromCodePoint(code) : match; });
...
export function renderFlowSvg(source: string): Promise<string> {
  ...
  void pending.catch(() => { if (cache.get(source) === pending) cache.delete(source); });
  return pending;
}
