...
export class LocalBrowserMemory implements BrowserMemory {
    ...
    try {
      ...
      if (value.openRequest != null) {
        ...
        if (
          ...
          !['dispatching', 'dispatched', 'failed', 'result-unknown'].includes(request.state) ||
          ...
          value.openRequest = {
            requestId:
              typeof request.requestId === 'string' ? request.requestId : 'invalid-local-request',
            threadId: value.targetThreadId,
            title: 'Existing open request',
            state: 'result-unknown',
            error:
              'The saved open request format could not be read. It will not run again automatically.',
          };
        }
      }
      return value;
  ...
  write(id: string, value: LocalWorkState) {
    this.storage().setItem(this.key(id), JSON.stringify(value));
  }
}
